...
| `403` | Token decoded but lacks the required organization context, **or** API-key auth was used on an endpoint that requires a Clerk session (currently [Revoke API key](/api/keys/revoke)). |
...
| 403 | `API key management requires a Clerk session. API keys cannot revoke API keys.` | [Revoke API key](/api/keys/revoke) when called with `sk_…` auth. |
