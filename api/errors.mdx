...
| 403 | `This endpoint requires a dashboard session. API keys are not permitted.` | [Create API key](/api/keys/create), [List API keys](/api/keys/list), and dashboard-internal endpoints (`/org/usage*`, `/org/scans/export`) when called with `sk_…` auth. |
