...
populating dashboards and review queues. Accepts either credential kind
— API keys (`sk_…`) and dashboard session tokens both work.
...
The History page's **Export** button downloads the current filter
result as CSV or Excel via an internal endpoint
(`GET /api/v2/org/scans/export`, capped at 10,000 rows). Unlike this
endpoint, export requires a dashboard session — API keys get `403`. To
export programmatically, paginate this endpoint instead.
