...
<Note>
  The dashboard also calls a few internal endpoints that are not part of
  the public API — the Usage page charts (`GET /org/usage` and its
  sub-routes) and the History page export (`GET /org/scans/export`).
  These require a dashboard session too and reject `sk_…` keys with the
  same `403`. They are UI implementation details, not a contract — don't
  build against them.
</Note>
...
| 403 | Token decodes but the active organization context is missing (session tokens only), **or** API-key auth was used on an endpoint that requires a dashboard session (the five management endpoints above, or a dashboard-internal endpoint), **or** the role is insufficient on an admin-only endpoint. |
