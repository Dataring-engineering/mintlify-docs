...
When a scan reaches a terminal status (`completed`,
...
<ResponseField name="dossier_url" type="string" required>
  Link to the detailed dossier for this scan.

  <Warning>
    Do not depend on this value. The hosted dossier viewer is not yet
    available; the field currently returns a placeholder path that
    will change. Fetch [`GET /api/v2/scans/{scan_id}`](/api/scans/get)
    directly for the canonical scan record.
  </Warning>
</ResponseField>
...
<ResponseField name="partial_reason" type="string">
  Present when `status` is `completed_with_partial`. Brief description
  of what was skipped.
</ResponseField>
